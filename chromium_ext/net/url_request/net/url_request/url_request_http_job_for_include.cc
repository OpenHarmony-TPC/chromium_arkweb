...
bool URLRequestHttpJob::MaybeRetryWithFallbackProxy(int& result) {
  if (request_->IsRetryingWithFallbackProxy()) {
    // Do not retry repeatedly while in the FallbackProxy process.
    return false;
  }
...
bool URLRequestHttpJob::SupportedHostAndNotUsedSystemProxy(
  ...
  if (proxy_delegate->IsFallbackProxyFailedHost(request_->url().GetHost())) {
...
void URLRequestHttpJob::RetryWithFallbackProxy() {
  LOG(INFO) << "URLRequestHttpJob::RetryWithFallbackProxy";
