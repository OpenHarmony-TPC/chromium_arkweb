...
bool IsPartialDrawEnable() {
  ...
  if (flag < 0) {
    ...
    std::string res = system_properties.GetStringParameter("web.gpu.partial.draw", "none");
    flag = (res == "single") || (res == "multi");
  }
...
bool NativeViewGLSurfaceEGL::SetPresentBufferDamage(gfx::Rect damage_rect, gfx::Rect curr_rect) {
  ...
  std::vector<EGLint> rects = {damage_rect.x(), GetSize().height() - damage_rect.y() - damage_rect.height(),
                damage_rect.width(), damage_rect.height()};
  EGLBoolean retVal = eglSetDamageRegionKHR(display_->GetDisplay(), surface_,
                  static_cast<EGLint*>(rects.data()),
...
void NativeViewGLSurfaceEGL::ClosePostSubBuffer() {
  LOG(INFO) << "ClosePostSubBuffer";
