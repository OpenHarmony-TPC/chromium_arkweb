...
基于原生Chromium扩展了如下功能：
   - **base基础库扩展：**
     - 深度路径集成：替换 Chromium 标准路径系统，完全遵循 OHOS 应用存储规范
     - 专业性能监控：相比 Chromium 基础监控，提供精确到 6 帧/12 帧/24 帧的微卡顿检测
     - LTPO 显示技术：支持 LTPO 动态帧率调整和分布式 VSync
     - 消息循环重构：重写事件循环，与 OHOS 原生事件处理机制深度集成
   - **图形渲染扩展：**
     - 更多图像格式：原生 Chromium 仅支持基础图像格式，ArkWeb 增加了 HEIF/HEIC 硬件解码支持，支持 4K 实时解码
     - 智能渲染控制：新增 VideoFrameSubmitter 精确控制，相比原生 Chromium 的粗粒度控制，可实现帧级别的功耗优化
     - 原生解码集成：直接调用 OHOS 系统解码器， bypass Chromium 软件解码路径
...
### 2.4 OS适配层 (`ohos_adapter`)
   ...
   - **图形系统适配器**
       - Native Window适配器封装OpenHarmony的原生窗口系统，提供窗口创建、管理和操作功能
       - VSync适配器封装垂直同步信号管理，确保图形渲染与显示器刷新率同步
       - Native Image适配器封装图像数据创建、处理和显示，支持 GPU 纹理集成
       - Ashmem适配器封装跨进程共享内存管理，为图形缓冲区提供高效的数据共享机制
   - **网络系统适配器**
       - 网连接适配器，封装网络连接状态监控和管理，提供实时的网络连接信息
       - 网络配置适配器，封装网络安全配置和策略管理，确保网络请求符合系统安全策略
       - 网络事件适配器，封装网络事件处理和分发，为 Web 引擎提供网络层事件通知
       - 网络代理适配器，封装网络代理配置和管理，支持各种代理模式
   - **多媒体适配器**
       - 网连接适配器，封装网络连接状态监控和管理，提供实时的网络连接信息
       - 网络配置适配器，封装网络安全配置和策略管理，确保网络请求符合系统安全策略
       - 网络事件适配器，封装网络事件处理和分发，为 Web 引擎提供网络层事件通知
   ...
   - **设备和服务适配器**
       ...
       - 电池管理适配器，电池状态监控和电池信息获取，电池事件监听。
       - 位置服务适配器，GPS 定位和位置信息获。支持不同场景、精度、优先级的定位配置，支持定位会话管理，能处理和转换位置数据格式。
