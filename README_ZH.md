...
基于原生CEF扩展了如下功能：
   ...
   - **网络与存储增强：**
     ...
     - 离线缓存：增强的离线页面缓存机制
   - **用户交互体验增强：**
     - 智能填充：支持H5页面表单中的护照（护照号、护照有效期、签发地），发票抬头（抬头名称，税号），车牌号，所在地区，航班号等类型的自动填充。支持H5验证码输入框自动拉起小艺数字键盘。
     - 跨设备同步：支持多设备间的填充数据同步
     - 触摸优化：支持多点触控、手势识别、触摸反馈
     - 传感器集成：加速计、陀螺仪等传感器支持
   - **高级渲染与显示：**
...
基于原生Chromium扩展了如下功能：
   - **base基础库扩展：**
     ...
     - LTPO 显示技术：支持 LTPO 动态帧率调整，对接系统Vsync，对接Frame Timing Management，提供DVsync
     - 消息循环重构：重写事件循环，与 OHOS 原生事件处理机制深度集成
   - **render图形渲染扩展：**
     - 图像格式：原生 Chromium 仅支持基础图像格式，ArkWeb增加了HEIF/HEIC 硬件解码支持，支持 4K 实时解码
