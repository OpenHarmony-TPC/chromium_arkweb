...
int __wrap_OH_CommonEvent_GetIntFromParameters(
...
OH_AudioStream_Result __wrap_OH_AudioStreamBuilder_SetChannelCount(
    ...
    OH_AudioStreamBuilder *builder, OH_AudioCapturer **audioCapturer)
{
    if (MockAudioCommonEventSupport::generateCapturer) {
        return MockAudioCommonEventSupport::GetInstance().OH_AudioStreamBuilder_GenerateCapturer(
            builder, audioCapturer);
    }
...
int32_t __wrap_OH_SensorSubscriptionAttribute_SetSamplingInterval(Sensor_SubscriptionAttribute* attr,
                                                                  int64_t samplingInterval) {
    if (MockOhSensorSupport::enableSubscriptionAttributeSetSamplingInterval) {
        return MockOhSensorSupport::getInstance().OH_SensorSubscriptionAttribute_SetSamplingInterval(attr,
                                                                                                     samplingInterval);
    } else {
...
Sensor_Result __wrap_OH_Sensor_Subscribe(const Sensor_SubscriptionId* id, const Sensor_SubscriptionAttribute* attr,
