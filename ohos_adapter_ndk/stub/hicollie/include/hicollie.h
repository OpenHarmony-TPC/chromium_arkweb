...
typedef enum HiCollie_ErrorCode {
     ...
     * @since 24
     */
    HICOLLIE_REACH_REPORT_LIMIT = 29800007,
} HiCollie_ErrorCode;
 ...
 * @since 24
 */
typedef enum HiCollie_Freeze_Type {
    /** Main thread watchdog timeout for one period. */
    THREAD_BLOCK_3S,
    /** Main thread watchdog timeout for two period. */
    THREAD_BLOCK_6S,
    /** Ability lifecycle timeout for one period. */
    LIFECYCLE_HALF_TIMEOUT,
    /** Ability lifecycle timeout for two period. */
    LIFECYCLE_TIMEOUT,
    /** Input event process timeout. */
    APP_INPUT_BLOCK,
    /** Freeze event report by {@link OH_HiCollie_Report}. */
    BUSINESS_THREAD_BLOCK_3S,
    /** Freeze event report by {@link OH_HiCollie_Report}. */
    BUSINESS_THREAD_BLOCK_6S,
    /** Freeze event report by {@link OH_HiCollie_ReportInputBlock}. */
    BUSINESS_INPUT_BLOCK
} HiCollie_Freeze_Type;
 ...
 * @since 24
 */
typedef enum HiCollie_FreezeCallback_Flag {
    /** Callback when intercept freeze event report by {@link OH_HiCollie_Report} or 
        {@link OH_HiCollie_ReportInputBlock}. */
    CALLBACK_WHEN_DEV_DETECTED,
    /** Callback when intercept freeze event report by system. */
    CALLBACK_WHEN_SYS_DETECTED
} HiCollie_FreezeCallback_Flag;
 
/**
 * @brief Set callback when system or developer detect freeze event
 *
 * @param type Freeze event type in {@link HiCollie_Freeze_Type}
 * @param buffer log buffer provided by system which content will be move to APP_FREEZE or APP_HICOLLIE hiappevent 
 * @param size Business thread processing event name.
 * @return used buffer size
 * @since 24
 */
typedef size_t (*OH_HiCollie_FreezeCallback)(HiCollie_Freeze_Type type, void* buffer, size_t size);
 
/**
 * @brief Set freeze callback to system, system will callback when
 *
 * @param callback The function callback {@link OH_HiCollie_FreezeCallback}.
 * @param flag The callback timing {@link HiCollie_FreezeCallback_Flag}.
 ...
 *
 * @return {@link HICOLLIE_SUCCESS} 0 - Success.
 *         {@link HICOLLIE_REACH_REPORT_LIMIT} 29800007 - report too frequently
 * @since 24
 */
